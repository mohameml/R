...
#ggplot est une bibliothèque graphique à connaître
if(!("ggplot2" %in%
     rownames(installed.packages()))){install.packages("ggplot2")}
library(ggplot2)
#ggplot(Mesures, aes(x = masse)) + geom_histogram()
#Pas le même calcul de la largeur des classes par défaut. Dans ggplot2, la
#largeur des classes (binwidth) est égale à l'étendue divisée par 30.
...
#Autre manière de spécifier le titre et le noms des axes
...
dev.off()
#Si besoin, créer des abréviations pour les noms des variables
...
#En plus pour supprimer la modalité <NA>
(table.graines.espece <-
    table(factor(Mesures5$graines),Mesures5$espece,dnn=c("nbr.graines","espece"),
...
barplot<-barplot(cumsum(histo$counts),ylab="Effectif",xlab="Masse",main="
                 Polygone des effectifs cumulés des masses")
...
#Effectifs et polygone des fréquences cumulées
library(qcc)
...
g +
  ...
  min(Mesures$masse)) + xlab("Masse") + ylab("Fréquence")
...
g +
  ...
  binwidth=diff(range(Mesures$masse))/nclass.Sturges(Mesures$masse),boundary =
  min(Mesures$masse)) + xlab("Masse") + ylab("Dénombrement")
...
#mads par rapport à une autre référence
...
#Autre example de calculs à partir d'un petit échantillon
x <- c(1,2,3,5,7,8)
sort(abs(x - median(x)))
c(mad(x, constant = 1),
...
#page 154
# Asymétrie et aplatissement d'un échantillon
if(!("agricolae" %in%
     rownames(installed.packages()))){install.packages("agricolae")}
library(agricolae)
skewness(Mesures$masse)
kurtosis(Mesures$masse)
#Pour retirer la bibliothèque agricolae de la mémoire de R avant de charger e1071
detach(package:agricolae)
if(!("e1071" %in% rownames(installed.packages()))){install.packages("e1071")}
library(e1071)
# Asymétrie et aplatissement d'une série statistique (=population)
skewness(Mesures$masse,type=1)
kurtosis(Mesures$masse,type=1)
# Asymétrie et aplatissement d'un échantillon (comme agricolae)
...
histo<-hist(Femmes,breaks=c(104,114,124,134,144,154,164,174,184),
            main="Histogramme de la variable taux d'hémoglobine pour les
            Femmes",
...
#Même résultats avec la fonction median
median(Ensemble)
median(Femmes)
median(Hommes)
